...
class MessagesExample : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContent {
            ComposeTutorialHW1Theme {
                Conversation(SampleDataExample.conversationSampleExample)
            }
        }
    }
}

data class ExampleMessage(val author: String, val body: String)

@Composable
fun MessageCard(msg: ExampleMessage) {
    //Padding
...
@Composable
fun Conversation(messages: List<ExampleMessage>) {
    LazyColumn {
...
fun PreviewMessageCard() {
    ComposeTutorialHW1Theme() {
        Surface {
            MessageCard(ExampleMessage("Hisik", "Coursework is like an any% speedrun"))
        }
    }
}
